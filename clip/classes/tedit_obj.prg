...
local bp,bl,l,i,dev, str, str1, block, min, max, fnd, clr_st
	...
	::colWin := min(min(max(1,::colWin),::nRight-::nLeft+1),::pos)
	   ...
	   if ::Nstyle .and. (bl+i) < ::lines+iif(::lEofString,1,0)
		...
		/*
		utf-8:
		@ ::ntop+i-1, ::nLeft-::__LNstyle say str(bl+i, ::__LNstyle-1, 0)+"‚îÇ" color clr_st
		*/
	   endif
...
local x, y, a1, a2, let, p, i
	x := len
	y := len
	let := "[¡≈…œ’Ÿ‹¿—aeiouy]" // utf-8: "[–∞–µ–∏–æ—É—ã—ç—é—èaeiouy]"
	spl := iif(spl==NIL, .f., .t.)
...
local ch, p
	...
	p := at(around_char[1], "á¥ß≠ÉâàÄä∫∑ΩÖ≤¶µ®Æ¨ª∏†æπ∂º") // utf-8: "‚î§‚ï¢‚ïñ‚ïú‚îê‚î¥‚î¨‚îÄ‚îº‚ï®‚ï•‚ï´‚îò‚ï°‚ïï‚ï£‚ïó‚ïù‚ïõ‚ï©‚ï¶‚ïê‚ï¨‚ïß‚ï§‚ï™"
	if p>=1 .and. p<14
		R_Mode=1
	endif
	if p >13
	...
	p = at(around_char[2], "ÑâàÜÄä∞∫∑™§ΩÇØ´•ª∏±†æπ∂©¢º") // utf-8: "‚îî‚î¥‚î¨‚îú‚îÄ‚îº‚ïü‚ï®‚ï•‚ïô‚ïì‚ï´‚îå‚ïû‚ïö‚ïî‚ï©‚ï¶‚ï†‚ïê‚ï¨‚ïß‚ï§‚ïò‚ïí‚ï™"
	if p >=1 .and. p<14
		L_Mode=1
	endif
	if p >13
	...
	p := at(around_char[3], "Åá≤¶ÉàÜäØ∂¢ºÇ¥ßµ°®∞•∏±æ∑§Ω") // utf-8:"‚îÇ‚î§‚ï°‚ïï‚îê‚î¨‚îú‚îº‚ïû‚ï§‚ïí‚ï™‚îå‚ï¢‚ïñ‚ï£‚ïë‚ïó‚ïü‚ïî‚ï¶‚ï†‚ï¨‚ï•‚ïì‚ï´" 
	if p >=1 .and. p <14
		U_Mode = 1
	endif
	if p >13
	...
	p := at(around_char[4], "Åá≤¨ÑâÜäØ©ºÖπ¥µ°Æ≠∞´ª±æ∫™Ω") // utf-8: "‚îÇ‚î§‚ï°‚ïõ‚îî‚î¥‚îú‚îº‚ïû‚ïò‚ï™‚îò‚ïß‚ï¢‚ï£‚ïë‚ïù‚ïú‚ïü‚ïö‚ï©‚ï†‚ï¨‚ï®‚ïô‚ï´"
	if p >=1 .and. p < 14
		D_Mode=1
	endif
	if p >13
...
local p, ch_ret, line, pos, old_direction:=0, str, ch, i, j
	undo := iif(undo == NIL, .t., undo)
	draw_char := {{"Ä", "†"}, {"Å", "°"}} /* uft-8: {{"‚îÄ", "‚ïê"}, {"‚îÇ", "‚ïë"}} */
	back_cur_ch := {"Éß¶®Ö¨≠Æá¥µ≤",; /* uft-8: "‚îê‚ïñ‚ïï‚ïó‚îò‚ïõ‚ïú‚ïù‚î§‚ï¢‚ï£‚ï°" */
			"Ç§¢•Ñ©™´Ü∞±Ø",; /* uft-8: "‚îå‚ïì‚ïí‚ïî‚îî‚ïò‚ïô‚ïö‚îú‚ïü‚ï†‚ïû" */
			"É¶ß®Ç§¢•à∂∏∑",; /* uft-8: "‚îê‚ïï‚ïñ‚ïó‚îå‚ïì‚ïí‚ïî‚î¨‚ï§‚ï¶‚ï•" */
			"Ö¨≠ÆÑ™©´âπª∫"}  /* uft-8: "‚îò‚ïõ‚ïú‚ïù‚îî‚ïô‚ïò‚ïö‚î¥‚ïß‚ï©‚ï®" */
	back_new_ch := {"à∑à∑ââ∫∫äΩΩä∏∏∂∏ªπªªæææº",; /* uft-8: "‚î¨‚ï•‚î¨‚ï•‚î¥‚î¥‚ï®‚ï®‚îº‚ï´‚ï´‚îº‚ï¶‚ï¶‚ï§‚ï¶‚ï©‚ïß‚ï©‚ï©‚ï¨‚ï¨‚ï¨‚ï™" */
			"à∑à∑ââ∫∫äΩΩä∏∏∂∏ªπªªæææº",; /* uft-8: "‚î¨‚ï•‚î¨‚ï•‚î¥‚î¥‚ï®‚ï®‚îº‚ï´‚ï´‚îº‚ï¶‚ï¶‚ï§‚ï¶‚ï©‚ïß‚ï©‚ï©‚ï¨‚ï¨‚ï¨‚ï™" */
			"á≤á≤ÜÜØØäººäµµ¥µ±∞±±æææΩ",; /* uft-8: "‚î§‚ï°‚î§‚ï°‚îú‚îú‚ïû‚ïû‚îº‚ï™‚ï™‚îº‚ï£‚ï£‚ï¢‚ï£‚ï†‚ïü‚ï†‚ï†‚ï¨‚ï¨‚ï¨‚ï´" */
			"á≤á≤ÜÜØØäººäµµ¥µ±∞±±æææΩ"}  /* uft-8: "‚î§‚ï°‚î§‚ï°‚îú‚îú‚ïû‚ïû‚îº‚ï™‚ï™‚îº‚ï£‚ï£‚ï¢‚ï£‚ï†‚ïü‚ï†‚ï†‚ï¨‚ï¨‚ï¨‚ï´" */
	string :={;
   /* present_derection=A_Right*/;
   /*up   */       {{"Ñ©Ñ©Ö¨Ñ©ÖÆâªÖÆÑ©Ö¨Ñ¨âπ","™´™´™´™´≠Æ∫ª≠Æ™´≠Æ™Æ∫ª"},; /* uft-8: "‚îî‚ïò‚îî‚ïò‚îò‚ïõ‚îî‚ïò‚îò‚ïù‚î¥‚ï©‚îò‚ïù‚îî‚ïò‚îò‚ïõ‚îî‚ïõ‚î¥‚ïß","‚ïô‚ïö‚ïô‚ïö‚ïô‚ïö‚ïô‚ïö‚ïú‚ïù‚ï®‚ï©‚ïú‚ïù‚ïô‚ïö‚ïú‚ïù‚ïô‚ïù‚ï®‚ï©" */
   /*down */        {"Ç¢Ç¢É¶Ç¢É®à∏É®Ç¢É¶Ç¶à∂","§•§•§•§•ß®∑∏ß®§•ß®§®∑∏"},; /* uft-8: "‚îå‚ïí‚îå‚ïí‚îê‚ïï‚îå‚ïí‚îê‚ïó‚î¨‚ï¶‚îê‚ïó‚îå‚ïí‚îê‚ïï‚îå‚ïï‚î¨‚ï§","‚ïì‚ïî‚ïì‚ïî‚ïì‚ïî‚ïì‚ïî‚ïñ‚ïó‚ï•‚ï¶‚ïñ‚ïó‚ïì‚ïî‚ïñ‚ïó‚ïì‚ïó‚ï•‚ï¶" */
   /*up down*/      {"ÜØÜØØ±ÜØáµäæáµÜØ≤≤≤≤ºæ","∞±∞±∞±∞±¥µΩæ¥±∞±áµáµäæ"};  /* uft-8: "‚îú‚ïû‚îú‚ïû‚ïû‚ï†‚îú‚ïû‚î§‚ï£‚îº‚ï¨‚î§‚ï£‚îú‚ïû‚ï°‚ï°‚ï°‚ï°‚ï™‚ï¨","‚ïü‚ï†‚ïü‚ï†‚ïü‚ï†‚ïü‚ï†‚ï¢‚ï£‚ï´‚ï¨‚ï¢‚ï†‚ïü‚ï†‚î§‚ï£‚î§‚ï£‚îº‚ï¨" */
		     },;
   /* present_derection=A_Left*/;
		   {{"Ö¨Ñ´Ñ©Ö¨Ö¨âªÖ©Ö¨Ö¨Ñ´âπ","≠Æ™´™´≠Æ≠Æ∫ª≠´≠Æ≠Æ™´∫ª"},; /* uft-8: "‚îò‚ïõ‚îî‚ïö‚îî‚ïò‚îò‚ïõ‚îò‚ïõ‚î¥‚ï©‚îò‚ïò‚îò‚ïõ‚îò‚ïõ‚îî‚ïö‚î¥‚ïß","‚ïú‚ïù‚ïô‚ïö‚ïô‚ïö‚ïú‚ïù‚ïú‚ïù‚ï®‚ï©‚ïú‚ïö‚ïú‚ïù‚ïú‚ïù‚ïô‚ïö‚ï®‚ï©" */
		    {"É¶Ç•Ç¢É¶É¶à∏É¢É¶Ç¢Ç•à∂","ß®§•§•ß®ß®∑∏ß•ß®ß®§•∑∏"},; /* uft-8: "‚îê‚ïï‚îå‚ïî‚îå‚ïí‚îê‚ïï‚îê‚ïï‚î¨‚ï¶‚îê‚ïí‚îê‚ïï‚îå‚ïí‚îå‚ïî‚î¨‚ï§","‚ïñ‚ïó‚ïì‚ïî‚ïì‚ïî‚ïñ‚ïó‚ïñ‚ïó‚ï•‚ï¶‚ïñ‚ïî‚ïñ‚ïó‚ïñ‚ïó‚ïì‚ïî‚ï•‚ï¶" */
		    {"á≤Ü±ØØá≤á≤äæáØá≤≤µÜ±ºæ","¥µ∞±Ü±¥µ¥µΩæÜ±¥µ¥µ∞µäæ"};  /* uft-8: "‚î§‚ï°‚îú‚ï†‚ïû‚ïû‚î§‚ï°‚î§‚ï°‚îº‚ï¨‚î§‚ïû‚î§‚ï°‚ï°‚ï£‚îú‚ï†‚ï™‚ï¨","‚ï¢‚ï£‚ïü‚ï†‚îú‚ï†‚ï¢‚ï£‚ï¢‚ï£‚ï´‚ï¨‚îú‚ï†‚ï¢‚ï£‚ï¢‚ï£‚ïü‚ï£‚îº‚ï¨" */
		     },;
   /* present_derection=A_Up*/;
   /*left   */     {{"Ö≠É®ÉßÖ≠Ö≠áµÖßÖ≠Ö≠É®á¥","¨Æ¶®¶®¨Æ¨Æ≤µ¨®¨Æ¨Æ¶®≤µ"},; /* uft-8: "‚îò‚ïú‚îê‚ïó‚îê‚ïñ‚îò‚ïú‚îò‚ïú‚î§‚ï£‚îò‚ïñ‚îò‚ïú‚îò‚ïú‚îê‚ïó‚î§‚ï¢","‚ïõ‚ïù‚ïï‚ïó‚ïï‚ïó‚ïõ‚ïù‚ïõ‚ïù‚ï°‚ï£‚ïõ‚ïó‚ïõ‚ïù‚ïõ‚ïù‚ïï‚ïó‚ï°‚ï£" */
   /*right  */      {"Ñ™Ç•Ç§Ñ™Ñ™Ü±Ñ§Ñ™Ñ™Ç•Ü∞","©´¢•¢•©´©´Ø±©•©´©´¢•Ø±"},; /* uft-8: "‚îî‚ïô‚îå‚ïî‚îå‚ïì‚îî‚ïô‚îî‚ïô‚îú‚ï†‚îî‚ïì‚îî‚ïô‚îî‚ïô‚îå‚ïî‚îú‚ïü","‚ïò‚ïö‚ïí‚ïî‚ïí‚ïî‚ïò‚ïö‚ïò‚ïö‚ïû‚ï†‚ïò‚ïî‚ïò‚ïö‚ïò‚ïö‚ïí‚ïî‚ïû‚ï†" */
   /*left right*/   {"â∫à∏∑∑â∫â∫äæâ∑â∫∫ªà∏æΩ","πª∂∏à∏πªπªºæà∏πªπª∂ªäæ"};  /* uft-8: "‚î¥‚ï®‚î¨‚ï¶‚ï•‚ï•‚î¥‚ï®‚î¥‚ï®‚îº‚ï¨‚î¥‚ï•‚î¥‚ï®‚ï®‚ï©‚î¨‚ï¶‚ï¨‚ï´","‚ïß‚ï©‚ï§‚ï¶‚î¨‚ï¶‚ïß‚ï©‚ïß‚ï©‚ï™‚ï¨‚î¨‚ï¶‚ïß‚ï©‚ïß‚ï©‚ï§‚ï©‚îº‚ï¨" */
		     },;
   /* present_derection=A_Down*/;
		   {{"ÉßÉßÉßÉßÖÆáµÖÆÉßÖ≠É≠á¥","¶®¶®¶®¶®¨Æ≤µ¨Æ¶®¨Æ¶Æ≤µ"},; /* uft-8: "‚îê‚ïñ‚îê‚ïñ‚îê‚ïñ‚îê‚ïñ‚îò‚ïù‚î§‚ï£‚îò‚ïù‚îê‚ïñ‚îò‚ïú‚îê‚ïú‚î§‚ï¢","‚ïï‚ïó‚ïï‚ïó‚ïï‚ïó‚ïï‚ïó‚ïõ‚ïù‚ï°‚ï£‚ïõ‚ïù‚ïï‚ïó‚ïõ‚ïù‚ïï‚ïù‚ï°‚ï£" */
		    {"Ç§Ç§Ç§Ç§Ñ´Ü±Ñ´Ç§Ñ™Ç™Ü∞","¢•¢•¢•¢•©´Ø±©´¢•©´¢´Ø±"},; /* uft-8: "‚îå‚ïì‚îå‚ïì‚îå‚ïì‚îå‚ïì‚îî‚ïö‚îú‚ï†‚îî‚ïö‚îå‚ïì‚îî‚ïô‚îå‚ïô‚îú‚ïü","‚ïí‚ïî‚ïí‚ïî‚ïí‚ïî‚ïí‚ïî‚ïò‚ïö‚ïû‚ï†‚ïò‚ïö‚ïí‚ïî‚ïò‚ïö‚ïí‚ïö‚ïû‚ï†" */
		    {"à∑à∑∑∏à∑âªäæâªà∑∫∫à∫Ωæ","∂∏∂∏∂∏∂∏πªºæπ∏∂∏âªâªäæ"};  /* uft-8: "‚î¨‚ï•‚î¨‚ï•‚ï•‚ï¶‚î¨‚ï•‚î¥‚ï©‚îº‚ï¨‚î¥‚ï©‚î¨‚ï•‚ï®‚ï®‚î¨‚ï®‚ï´‚ï¨","‚ï§‚ï¶‚ï§‚ï¶‚ï§‚ï¶‚ï§‚ï¶‚ïß‚ï©‚ï™‚ï¨‚ïß‚ï¶‚ï§‚ï¶‚î¥‚ï©‚î¥‚ï©‚îº‚ï¨" */
		    };
		   }

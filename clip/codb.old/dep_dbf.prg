...
static function _dep_append(self,oData,class_id,obj_id)
	...
	//outlog(__FILE__,__LINE__,class_id,class_desc)
...
static function _dep_select(self,class_Id,nIndex,sName,sWhere,nCount,deleted)
	...
	taskstop()
	//outlog(__FILE__,__LINE__,deleted,s,aWrapNames)
...
static function _dep_makeIndies(self,lOut)
	...
	tmp := oDict:select("EXTENT")
	for i=1 to len(tmp)
		dbfile:=self:path+PATH_DELIM+tmp[i]
		ferase(dbfile+".cdx")
		hdb:=rddUseArea(CODB_DDRIVER_DEFAULT,dbfile,,.f.)
		if lOut
			? "create index",dbfile

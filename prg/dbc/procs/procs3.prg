...
LOCAL mssot  := { "сто", "двести", "триста", "четыреста", "пятьсот", "шестьсот", "семьсот", "восемьсот", "девятьсот" }
LOCAL msdes  := { "двадцать", "тридцать", "сорок", "пятьдесят", "шестьдесят", "семьдесят", "восемьдесят", "девяносто" }
LOCAL ms1des := { "десять", "одиннадцать", "двенадцать", "тринадцать", "четырнадцать", "пятнадцать", "шестнадцать", "семнадцать", "восемнадцать", "девятнадцать" }
LOCAL msed   := { "один", "два", "три", "четыре", "пять", "шесть", "семь", "восемь", "девять" }
LOCAL msed1  := { "одна", "две" }
LOCAL msrod  := { .T., .T., .F., .T. }
   valut := IIF( valut = Nil, .F., valut )
   msnm1[ 1 ] = "миллиард"
   msnm1[ 2 ] = "миллион"
   msnm1[ 3 ] = "тысяча"
   msnm1[ 4 ] = IIF( valut, "доллар США", "рубль" )
   msnm2[ 1 ] = "миллиарда"
   msnm2[ 2 ] = "миллиона"
   msnm2[ 3 ] = "тысячи"
   msnm2[ 4 ] = IIF( valut, "доллара США", "рубля" )
   msnm3[ 1 ] = "миллиардов"
   msnm3[ 2 ] = "миллионов"
   msnm3[ 3 ] = "тысяч"
   msnm3[ 4 ] = IIF( valut, "долларов США", "рублей" )
   strupp := "АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯ"
   strlow := "абвгдежзийклмнопрстуфхцчшщьыъэюя"
   str2   := SPACE( 10 )
   str3   := SPACE( 10 )
   strrez := SPACE( 120 )
   len    := AT( ".", chislo ) - 1
   ...
   IF kod <> 0
      ...
      IF valut
         strrez := TRIM( strrez ) + " " + SUBSTR( chislo, len + 2, 2 ) + IIF( ( kod > 9 .AND. kod < 21 ) .OR. skop = 0 .OR. skop > 4, " центов", IIF( skop = 1, " цент", " цента" ) )
      ELSE
         strrez := TRIM( strrez ) + " " + SUBSTR( chislo, len + 2, 2 ) + IIF( ( kod > 9 .AND. kod < 21 ) .OR. skop = 0 .OR. skop > 4, " копеек", IIF( skop = 1, " копейка", " копейки" ) )
      ENDIF
   ELSE
      strrez := TRIM( strrez ) + "="
   ENDIF

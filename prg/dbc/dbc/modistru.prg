...
#else
LOCAL ctrl_ar := { "B/W,W/B", ;
                      { { "ESC - Выход",, 20, 22 }, { "ALT-M - Сохранить",, 20, 37 } } }
#endif
...
#else
   @  4, 26 SAY IIF( prnew, "Создание нового файла", "Структура " + msfile[ improc ] )
#endif
   OutCtrls( ctrl_ar )
   DO WHILE rez
      LI_LSOHR := .F.
...
PRIVATE ctrl_ar2 := { "+W/RB,+W/B", ;
                         { { "",, 7, 13, 15, 21 }, ;
                         { "Ins - Insert",, 17, 12 }, { "Del - Delete",, 17, 32 }, { "F6 - Убрать все поля",, 17, 46 } } }
#else
PRIVATE ctrl_ar1 := { "+W/RB,+W/B", ;
                         { { "",, 7, 37, 15, 55 }, ;
                         { "Ins - Выбрать поле",, 17, 12 }, { "F5 - Select all fields",, 17, 32 } } }
PRIVATE ctrl_ar2 := { "+W/RB,+W/B", ;
                         { { "",, 7, 13, 15, 21 }, ;
                         { "Ins - Вставить",, 17, 12 }, { "Del - Удалить",, 17, 32 }, { "F6 - Убрать все поля",, 17, 46 } } }
#endif
...
#else
   @ 09, 14 SAY "Введите выражение для столбца"
#endif
   @ 10, 14 GET expfiel PICTURE "@S50"
#ifdef ENGLISH
   @ 11, 14 SAY "Name:" GET txname
#else
   @ 11, 14 SAY "Имя:" GET txname
#endif
   READ
   SETCOLOR( oldc )
...
#else
   @ 09, 14 SAY "Имя:"  GET txname
   @ 10, 14 SAY "Выражение для столбца"
#endif
   @ 11, 14 GET expfiel PICTURE "@S50"
#ifdef ENGLISH
   @ 12, 14 SAY "Width:"  GET nWidth
#else
   @ 12, 14 SAY "Длина:"  GET nWidth
#endif
   READ
   SETCOLOR( oldc )

...
#endif
PUBLIC ORAMKA   := "⁄ƒø≥Ÿƒ¿≥ ", DRAMKA := "…Õª∫ºÕ»∫ ", klrecf := 200
PUBLIC txtinf   := ""
PRIVATE que_simpl := .T.
PUBLIC aMMenu[ 4 ]
...
#else
   aMMenu[ 2 ] = { { " î†©´ ",, 1, 2 }, { " à≠§•™· ",, 1, 10 }, { " èÆ´Ô ",, 1, 20 }, ;
		   { " Ñ¢®¶•≠®• ",, 1, 26 }, { " äÆ¨†≠§Î",, 1, 36 }, { " View ",, 1, 46 }, { " á†Ø‡Æ· ",, 1, 55 }, ;
		   { " Ñ‡„£Æ• ",, 1, 63 }, { " ÇÎÂÆ§",, 1, 72 } }
PRIVATE stra1 := "ìØ‡†¢´•≠®• xbase ‰†©´†¨®"
PRIVATE stra3 := "(C)1998,2001 Ä´•™·†≠§‡ ä‡•·®≠"
...
#ifdef __HARBOUR__
#ifdef RDD_AX
PRIVATE stra2 := "Ç•‡·®Ô 3.1b ( Harbour+ADS )"
#else
PRIVATE stra2 := "Ç•‡·®Ô 3.1b ( Harbour )"
#endif
#else
#ifdef RDD_AX
PRIVATE stra2 := "Ç•‡·®Ô 3.1b ( ADS )"
#else
PRIVATE stra2 := "Ç•‡·®Ô 3.1b"
#endif
#endif
#endif
   SET EXCLUSIVE ON
...
#else
LOCAL strerr := "ç• „§†´Æ·Ï Æ‚™‡Î‚Ï ‰†©´ " + Iif( fname != Nil, fname, Alias() )
#endif
...
#else
   bl0 := { | mslist | DEVPOS( _y2, LI_X1 + 2 ), DEVOUT( IIF( LI_PRFLT, STR( LI_TEKZP, 6 ), STR( RECNO(), 6 ) ) + ;
	    "/" + STR( LI_KOLZ, 6 ) + IIF( LI_PRFLT, " î®´Ï‚‡: " + DBFILTER(), "" ) ), DbflUsr( mslist ) }
#endif
   DO WHILE rez
      prfi := .T.
      IF improc > 0
...
#ifdef RDD_AX
#ifdef __HARBOUR__
LOCAL submenu := { " 1 NEW", " 2 OPEN       Alt-O", " 3 CLOSE", " 4 PRINT      Alt-P", "ƒ", ;
		      " 5SELECT AREA Alt-0", " 6 NEXT AREA  Alt->", " 7 PREV.AREA  Alt-<", "ƒ", ;
...
#ifdef RDD_AX
#ifdef __HARBOUR__
LOCAL submenu := { " 1 çéÇõâ", " 2 éíäêõíú    Alt-O", " 3 áÄäêõíú", " 4 èÖóÄíú     Alt-P", "ƒ", ;
		      " 5 ÇõÅéê éäçÄ Alt-0", " 6 ëãÖÑ.éäçé  Alt->", " 7 èêÖÑ.éäçé  Alt-<", "ƒ", ;
		      " 8 EXCLUSIVE    ON ", " 9 READONLY     OFF", " 0 AXS LOCK     ON ", " DRIVER: ADS_CDX", " ëÖêÇÖê:ãéäÄãúçõâ" }
#else
LOCAL submenu := { " 1 çéÇõâ", " 2 éíäêõíú    Alt-O", " 3 áÄäêõíú", " 4 èÖóÄíú     Alt-P", "ƒ", ;
		      " 5 ÇõÅéê éäçÄ Alt-0", " 6 ëãÖÑ.éäçé  Alt->", " 7 èêÖÑ.éäçé  Alt-<", "ƒ", ;
...
#endif
#else
LOCAL submenu := { " 1 çéÇõâ", " 2 éíäêõíú    Alt-O", " 3 áÄäêõíú", " 4 èÖóÄíú", "ƒ", ;
		      " 5 ÇõÅéê éäçÄ Alt-0", " 6 ëãÖÑ.éäçé  Alt->", " 7 èêÖÑ.éäçé  Alt-<", "ƒ", ;
		      " 8 EXCLUSIVE    ON ", " 9 READONLY     OFF", "   DRIVER: DBFCDX" }
#endif
#endif
...
#else
      submenu[ 14 ] = Stufmy( submenu[ 14 ], 9, 9, IIF( nServerType == 1, "ãéäÄãúçõâ", "ìÑÄãÖççõâ" ) )
#endif
#else
...
#else
LOCAL submenu := { " 1 ÇõÅéê íÖäìôÖÉé àHÑÖäëÄ ", " 2 ëéáÑÄíú çéÇõâ àçÑÖäë", ;
		      " 3 éíäêõíú àçÑÖäëçõâ îÄâã", " 4 áÄäêõíú àçÑÖäëõ" }
#endif
...
#ifdef __NOEXTRA__
#ifdef ENGLISH
LOCAL submenu := { " 1 VIEW STRUCTURE", " 2 EDIT STRUCTURE", "ƒ", " 3 EDIT RECORD        Alt-E", ;
		      "ƒ", " 4 SELECT COLUMNS OF BROWSE", " 5 INSERT COLUMN    Alt-Ins", " 6 DELETE COLUMN    Alt-Del", ;
		      " 7 EDIT COLUMN" }
#else
LOCAL submenu := { " 1 èêéëåéíê ëíêìäíìêõ", " 2 êÖÑÄäíàêéÇÄçàÖ ëíêìäíìêõ ", ;
		      "ƒ", " 3 êÖÑÄäí-Ö áÄèàëà    Alt-E", "ƒ", " 4 ÇõÅéê ëíéãÅñéÇ BROWSE   ", ;
		      " 5 ÇëíÄÇäÄ ëíéãÅñÄ  Alt-Ins", " 6 ìÑÄãÖçàÖ ëíéãÅñÄ Alt-Del", " 7 êÖÑÄäíàêéÇÄçàÖ ëíéãÅñÄ" }
#endif
...
LOCAL submenu := { " 1 VIEW STRUCTURE", " 2 EDIT STRUCTURE", "ƒ", " 3 EDIT RECORD        Alt-E", ;
		      "ƒ", " 4 SELECT COLUMNS OF BROWSE", " 5 INSERT COLUMN    Alt-Ins", " 6 DELETE COLUMN    Alt-Del", ;
		      " 7 EDIT COLUMN", "ƒ", " 8 COPY FIELD      Ctrl-Ins", " 9 INSERT FIELD   Shift-Ins" }
#else
LOCAL submenu := { " 1 èêéëåéíê ëíêìäíìêõ", " 2 êÖÑÄäíàêéÇÄçàÖ ëíêìäíìêõ ", ;
		      "ƒ", " 3 êÖÑÄäí-Ö áÄèàëà    Alt-E", "ƒ", " 4 ÇõÅéê ëíéãÅñéÇ BROWSE   ", ;
		      " 5 ÇëíÄÇäÄ ëíéãÅñÄ  Alt-Ins", " 6 ìÑÄãÖçàÖ ëíéãÅñÄ Alt-Del", " 7 êÖÑÄäíàêéÇÄçàÖ ëíéãÅñÄ", ;
		      "ƒ", " 8 äéèàêéÇÄíú èéãÖ Ctrl-Ins", " 9 ÇëíÄÇàíú èéãÖ  Shift-Ins" }
#endif
...
#else
	 @  3, 19 SAY "Ç·•£Æ ØÆ´•©: " + STR( FCOUNT(), 3 )                   
	 @  4, 19 SAY "Ñ´®≠† ß†Ø®·®" + STR( RECSIZE(), 6 ) + " °†©‚"         
#endif
      ENDIF
...
LOCAL submenu := { " 1 REPLACE", " 2 DELETE", " 3 RECALL", " 4 COUNT", " 5 SUM", ;
		      " 6 APPEND FROM", " 7 COPY TO", " 8 REINDEX", " 9 PACK", " 0 ZAP", "ƒ", " DO SCRIPT", "ƒ", " MEMO      ", " SET RELATION" }
...
#else
LOCAL submenu := { " 1 éíäêõíú", " 2 ëéïêÄçàíú" }
#endif
   DO WHILE .T.
      SET COLOR TO +GR/B,N/BG,,,N/B
...
#else
	IF .NOT.MsgYesNo(fname+" „¶• ·„È•·‚¢„•‚! è•‡•ß†Ø®·†‚Ï?")
#endif
	 rez=.T.
	ENDIF
       ENDIF
...
#else
LOCAL submenu := { " 1 çéÇõâ áÄèêéë", " 2 éíäêõíú áÄèêéë", " 3 êÖÑÄäíàêéÇÄíú", ;
		      " 4 ëéïêÄçàíú" }
#endif
#else
#ifdef ENGLISH
...
#else
LOCAL submenu := { " èÖêÖäãûóÖçàÖ ÇàÑÖéêÖÜàåÄ", " êÄëôÖèàíú(ëãàíú) ùäêÄç", ;
		      " ÇõóàëãÖçàÖ  ÇõêÄÜÖçàâ", " îéêåÄí ÑÄíõ", " CHARSET: DOS ", " éäçé Ñãü åÖåé      ON", " Çõèéãçàíú ëäêàèí" }
#endif
   DO WHILE .T.
#ifdef ENGLISH
...
LOCAL expkey := SPACE( 100 ), expfor := SPACE( 100 )
   ...
   IF msmode[ improc , 3 ] = 1
      ...
      @  9, 40 SAY "íag"            GET tagname PICTURE "XXXXXXXX" WHEN prmulti $ "Yy"         
   ENDIF
   @ 10, 22 SAY "Expression:"         
   @ 12, 22 SAY "Condition:"          
#else
   @  7, 25 SAY "ëÆß§†≠®• ®≠§•™·† §´Ô " + CutPath( msfile[ improc ] ) COLOR "+GR/GR"                                    
   @  8, 22 SAY "à¨Ô"                                      GET indname    PICTURE REPLICATE( 'X', 36 )       
   IF msmode[ improc , 3 ] = 1
      @  9, 22 SAY "MultiBag (Y/N)" GET prmulti PICTURE "X"        VALID prmulti $ "YyNn"      
      @  9, 40 SAY "íag"            GET tagname PICTURE "XXXXXXXX" WHEN prmulti $ "Yy"         
   ENDIF
...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       
                        §Å˘â  gÅ@Ö™?A:;=    Ä             ˆb                                                         †õˇ
                        §Å  ¨  _Å@Üâ™?Üâ™?    Ä             $c #c                                                     <                         §Å    _Å@áâ™?áâ™?    Ä             &c %c                                                     ‚                         ˇ°  '   `Å@áâ™?áâ™?    Ä              /home/rust/tmp/netpbm-10.18/pm_config.h                                              ˇ°  $   `Å@áâ™?áâ™?    Ä              /home/rust/tmp/netpbm-10.18/lib/pm.h                        7                         ˇ°  %   `Å@áâ™?áâ™?    Ä              /home/rust/tmp/netpbm-10.18/lib/pbm.h                       L                         ˇ°  -   `Å@áâ™?áâ™?    Ä              /home/rust/tmp/netpbm-10.18/lib/util/shhopt.h               b                         ˇ°  .   `Å@áâ™?áâ™?    Ä              /home/rust/tmp/netpbm-10.18/lib/util/nstring.h              w                         ˇ°  0   `Å@áâ™?áâ™?    Ä              /home/rust/tmp/netpbm-10.18/lib/util/mallocvar.h            å                         §Å  ¥  `Å@àâ™?àâ™?    Ä             Kc Lc                                                                               ÌÅ  …!  `Å@àâ™?àâ™?    Ä             Oc Pc Qc                                                 ô                         §Å  Ä  `Å@ââ™?ââ™?    Ä             Rc                                                         †                         ÌÅ  `  `Å@ââ™?ââ™?    Ä             Sc Tc                                                     Ì                         §Å  	  `Å@ââ™?ââ™?    Ä             Uc                                                         ˘                         ÌÅ  ?  aÅ@ââ™?ââ™?    Ä             Vc Wc                                                     I                         §Å  ÿ  aÅ@ââ™?ââ™?    Ä             Xc                                                         Z                         ÌÅ  !  aÅ@äâ™?äâ™?    Ä             ac bc cc                                                 ¨                         §Å  ê  aÅ@äâ™?äâ™?    Ä             ec dc                                                     ∑                         ÌÅ  .  aÅ@äâ™?äâ™?    Ä             fc gc hc                                                 
                         §Å  (	  aÅ@äâ™?äâ™?    Ä             õc                                                                                  ÌÅ    aÅ@äâ™?äâ™?    Ä             úc ùc                                                     `                         §Å  l  aÅ@ãâ™?ãâ™?    Ä             ûc                                                         ç                         ÌÅ  ß  aÅ@ãâ™?ãâ™?    Ä             üc †c                                                     ﬁ                         §Å  §  aÅ@ãâ™?ãâ™?    Ä             °c                                                         ‚                         ÌÅ      aÅ@ãâ™?ãâ™?    Ä             ¢c £c §c                                                 8                         §Å  t  aÅ@åâ™?åâ™?    Ä             ¶c •c                                                     I                         ÌÅ  ∆%  aÅ@åâ™?åâ™?    Ä             ßc ®c ©c                                                 õ                         §Å  Ä  aÅ@åâ™?åâ™?    Ä             ¨c                                                         Õ                         ÌÅ  ‹  aÅ@åâ™?åâ™?    Ä             ≠c Æc                                                                              §Å  p  aÅ@çâ™?çâ™?    Ä             Øc                                                         E                         ÌÅ  »   aÅ@çâ™?çâ™?    Ä             ∞c ±c ≤c                                                 ñ                         

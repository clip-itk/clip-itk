...
#else
LOCAL cMsg1 := "Запрос был изменен! Сохранить?"
#endif
...
#else
PRIVATE Ctrlist := { "+GR/B,+W/RB", { { "F5 - Выполнить запрос", { || DoQuery() }, 19, 10 } } }
#endif
PRIVATE GetList := {}
...
#else
LOCAL cMsg1 := "Сохранить предыдущий запрос?"
#endif
   IF que_upd
      IF MsgYesNo( cMsg1 )
         WriteQuery()
...
PRIVATE stroka, poz1, f_impr, f_usl := "", max_len, max_dec, msrelat := {}, msaccum
   msquery := ARRAY( 6 )
   // Поля из SELECT - в массив msquery[1]
   stroka       := textquery[ 1 ]
   poz1         := 1
   msquery[ 1 ] := {}
   ...
   ENDDO
   // Файлы из FROM - в массив msquery[2]
   stroka       := textquery[ 2 ]
   poz1         := 1
   msquery[ 2 ] := {}
   DO WHILE VALTYPE( sword := EL_NEXT() ) == "C"
   ...
   IF ( i := IsAgr( msquery[ 1 , 1 ] ) ) <> 0 .AND. groupbl = Nil
      // Если запрашивается агрегатная функция
...
#else
               MsgSay( "DISTINCT и GROUP BY в одном запросе!" )
#endif
               RETURN Nil
            ENDIF
...
#else
      MsgInf( "Ждите ..." + IIF( Ax_ExprValid( f_usl ), " Ok", " Hm.." ) )
#endif
#endif
#endif
      SET FILTER TO &f_usl
      ...
      DO WHILE .NOT. EOF()
         IF EVAL( msquery[ 3 ] )
            ...
            IF i <= max_len
               ...
               LOOP
            ENDIF
            IF groupbl <> Nil           // Если указан GROUP BY
               IF groupkey <> EVAL( groupbl )               // Если начинается новая группа
                  ...
                  FOR i := 1 TO LEN( msgr )
                     ...
                     ENDIF
                  NEXT
               ELSE                     // Если продолжается текущая группа
                  FOR i := 1 TO LEN( msgr )
                     IF msgr[ i ]
               ...
               ENDIF
            ELSE    //
               IF distbl <> Nil         // Если указан DISTINCT
         ...
         DO WHILE .T.
            IF ocher = max_len
               ...
               DO WHILE EVAL( msrelat[ ocher , 5 ] )
                  IF EVAL( msquery[ 3 ] )                   // Если выпоняется условие WHERE
                     IF groupbl <> Nil  // Если указан GROUP BY
                        IF groupkey <> EVAL( groupbl )      // Если начинается новая группа
                           IF groupkey <> "~~~" .AND. EVAL( msquery[ 5 ] )
                              SELECT 20
...
#else
      MsgSay( "Результат: " + TRANSFORM( msaccum, "@(" ), "R/W" )
#endif
   ELSE
      SELECT 20
      IF .NOT. EMPTY( ordexp )
...
#else
               MsgSay( sword + " - неизвестное слово в Having!" )
#endif
               RETURN ""
            ELSE
...
#else
         MsgSay( sword + " - Неизвестный алиас" )
#endif
         RETURN .F.
      ENDIF
   ELSE

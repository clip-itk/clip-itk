...
static function command_help(cont)
	local f:=getenv("CLIPROOT")
        ...
        f="file:"+f+[/doc/rus/debug.html] //+iif(!empty(cont),"#"+cont,"")
        hbrowse(,,,,f)
return .t.
************************************************
...
return .t.
************************************************
static function command_Connect()
	local x,in:=space(0), mp:={}
#ifdef __UNIX__
        syscmd("ps x","",@in)
        mp:=split(in,"&\n")
        adel(mp,1)
	...
	aadd(mp,[NonUnix is not supported !])
        ...
        if x!=0
        	...
        	::dbgconnect:=dbgconnect(val(mp[x]))
                ::command:put([Process connection OK]+":"+alltrim(str(val(mp[x]))) )

...
static com_names:={;
    {"rem","#", "*", "//" } ;
   ,{"I","У","INPUT", "УПРАВЛЕНИЕ" } ;
   ,{"M","М","MONITOR","МОНИТОР"} ;
   ,{ "?","??" } ;
   ,{"A","А","ANIMATE","АНИМАЦИЯ" } ;
   ,{"B","BP","ТО","BREAKPOINT" } ;
   ,{"C","ВЫЗОВ","CALLSTACK" } ;
   ,{"D","Д","УДАЛИТЬ","DELETE" } ;
   ,{"DOS","OS" } ;
...
function dbg_viewvar(vname,level,p,flag)
	...
	*********
	...
	do case
		...
		case t=="A"
			for i=1 to len(p)
				aadd(m,str(i,4,0)+"�"+::viewVar(vname,level,p[i],.f.)) // utf-8: "│"
			next
			do while !empty(m)
		...
		case t=="O"
			...
			for i=1 to len(mv)
				...
				aadd(m,padr(h,15)+"�"+::viewVar(vname,level,p[mv[i]],.f.) ) // utf-8: "│"
			next
			do while !empty(m)
				curPos=achoice(0,0,maxrow(),maxcol(),m,,,curPos,curPos)

...
#IFDEF FT_TEST
   ...
   * Border, Box, Bar, Current, Unselected
   ...
   SETCOLOR( cNormN )
   @ nMaxRow, 0
   @ nMaxRow, 0 SAY " FT_MENU1 1.0 ≥ " // utf-8: " FT_MENU1 1.0 ‚îÇ "
   @ NMAXROW,16 SAY "WRITTEN BY PAUL FERRARA [76702,556] FOR NANFORUM.LIB"
   @ NMAXROW,69 SAY "≥ "+DTOC( DATE() ) // utf-8: "‚îÇ "

   SETCOLOR( cErrH )
   @ nMaxRow-11, 23, nMaxRow-3, 56 BOX "⁄ƒø≥Ÿƒ¿≥ " // utf-8: "‚îå‚îÄ‚îê‚îÇ‚îò‚îÄ‚îî‚îÇ "
   @ nMaxRow- 9,23 SAY "√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥" // utf-8: "‚îú‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î§"
   SETCOLOR( cErrN )
...
FUNCTION FT_MENU1( aBar, aOptions, aColors, nTopRow, lShadow )
   ...
   DO WHILE lLooping
      ...
      ENDIF
      SETCOLOR( cBorder )
      @  nTopRow+1, aBoxLoc[nHpos], LEN(aChoices[nHpos,1])+nTopRow+2, aBarWidth[nHpos]+3+aBoxLoc[nHpos] BOX "…Õª∫ºÕ»∫ " // utf-8: "‚ïî‚ïê‚ïó‚ïë‚ïù‚ïê‚ïö‚ïë "

...
#define K_DIVIDE   47
#define K_ZERO     48
#define B_DOUBLE '…Õª∫ºÕ»∫ ' // utf-8: '‚ïî‚ïê‚ïó‚ïë‚ïù‚ïê‚ïö‚ïë '
#define B_SINGLE '⁄ƒø≥Ÿƒ¿≥ ' // utf-8: '‚îå‚îÄ‚îê‚îÇ‚îò‚îÄ‚îî‚îÇ '
...
STATIC FUNCTION _ftAddScreen(aAdder)
  ...
  nCol := 5+nAddSpace
  @  7+nTopOS, nCol SAY '      ⁄ƒƒƒø ⁄ƒƒƒø ⁄ƒƒƒø' // utf-8:    '      ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê'
  @  8+nTopOS, nCol SAY '      ≥   ≥ ≥   ≥ ≥   ≥' // utf-8: '      ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ'
  @  9+nTopOS, nCol SAY '      ¿ƒƒƒŸ ¿ƒƒƒŸ ¿ƒƒƒŸ' // utf-8: '      ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò'
  @ 10+nTopOS, nCol SAY '⁄ƒƒƒø ⁄ƒƒƒø ⁄ƒƒƒø ⁄ƒƒƒø' // utf-8:     '‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê'
  @ 11+nTopOS, nCol SAY '≥   ≥ ≥   ≥ ≥   ≥ ≥   ≥' // utf-8: '‚îÇ   ‚îÇ ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ'
  @ 12+nTopOS, nCol SAY '¿ƒƒƒŸ ¿ƒƒƒŸ ¿ƒƒƒŸ ≥   ≥' // utf-8: '‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îÇ   ‚îÇ' 
  @ 13+nTopOS, nCol SAY '⁄ƒƒƒø ⁄ƒƒƒø ⁄ƒƒƒø ≥   ≥' // utf-8:    '‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îÇ   ‚îÇ'
  @ 14+nTopOS, nCol SAY '≥   ≥ ≥   ≥ ≥   ≥ ≥   ≥' // utf-8: '‚îÇ   ‚îÇ ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ'
  @ 15+nTopOS, nCol SAY '¿ƒƒƒŸ ¿ƒƒƒŸ ¿ƒƒƒŸ ¿ƒƒƒŸ' // utf-8: '‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò'
  @ 16+nTopOS, nCol SAY '⁄ƒƒƒø ⁄ƒƒƒø ⁄ƒƒƒø ⁄ƒƒƒø' // utf-8:     '‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê'
  @ 17+nTopOS, nCol SAY '≥   ≥ ≥   ≥ ≥   ≥ ≥   ≥' // utf-8: '‚îÇ   ‚îÇ ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ'
  @ 18+nTopOS, nCol SAY '¿ƒƒƒŸ ¿ƒƒƒŸ ¿ƒƒƒŸ ≥   ≥' // utf-8: '‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îÇ   ‚îÇ'
  @ 19+nTopOS, nCol SAY '⁄ƒƒƒƒƒƒƒƒƒø ⁄ƒƒƒø ≥   ≥' // utf-8:   '‚îå‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îê ‚îå‚îÄ‚îÄ‚îÄ‚îê ‚îÇ   ‚îÇ'
  @ 20+nTopOS, nCol SAY '≥         ≥ ≥   ≥ ≥   ≥' // utf-8: '‚îÇ         ‚îÇ ‚îÇ   ‚îÇ ‚îÇ   ‚îÇ'
  @ 21+nTopOS, nCol SAY '¿ƒƒƒƒƒƒƒƒƒŸ ¿ƒƒƒŸ ¿ƒƒƒŸ' // utf-8: '‚îî‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò ‚îî‚îÄ‚îÄ‚îÄ‚îò'

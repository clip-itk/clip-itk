...
FUNCTION FT_MMICKEYS( nX, nY ) // read mouse motion counters
   ...
   FT_INT86( 51, aReg )        // execute mouse interrupt
   nX := aReg[CX]               // store horizontal motion units
   nY := aReg[DX]               // store vertical motion units
...
nMinor=LowByte(aReg[BX])
nType=HighByte(aReg[CX])
nIRQ=LowByte(aReg[CX])

// Return

RETURN HighByte(aReg[BX])

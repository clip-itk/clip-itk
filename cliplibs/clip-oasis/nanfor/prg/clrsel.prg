 ...
 *                 Default is CHR(177) "‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí‚ñí"
 *
 *
...
DO WHILE .T.
  ...
  *.... show sample window
  ...
  IF aOpt[ C_TYPE ] <> "T"  // no prompt for titles
    ...
    Double( nT, nL+1, nB, nR-1 )
    @ nT, nL+2 SAY PadC( " "+ aOpt[C_NAME] +" ", nR -nL -3, "Õ" )
  ...
  *.... allow change to specific part of colour string
  IF aOpt[ C_TYPE ] <> "T"
    Single( nT, nL+1, nB, nR-1 )
    @ nT, nL+2 SAY PadC( " "+ aOpt[C_NAME] +" ", nR -nL -3, "ƒ" ) // utf-8: "‚îÄ"
  ENDIF
...
DO CASE
   ...
   CASE aOpt[ C_TYPE ] == "G"    // Get windows
     ...
     @ 21,42 SAY "√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥" // utf-8: "‚îú‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î§"
     @ 22,43 SAY  "   Amount            "
     @ 23,43 SAY  "   Date              "
     SETCOLOR( aClr[2] )
     @ 22,53 SAY             "  199.95"
   ...
   CASE aOpt[ C_TYPE ] == "W"    // Alert windows
     ...
     @ 21,42 SAY  "                       "
     @ 22,41 SAY "√ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥" // utf-8: "‚îú‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚î§"
     SETCOLOR( aClr[2] )
     @ 23,44 SAY  " Accept "
     SETCOLOR( aClr[5] )
     @ 23,55 SAY             " Reject "
   ...
   CASE aOpt[ C_TYPE ] == "B"    // browse windows
     ...
     @ 20,38 SAY "∆ÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕµ" // utf-8: "‚ïû‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ï§‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ï§‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ï°"
     @ 21,39 SAY  "  312 ≥ Rick Shaw    ≥ 143.25 "
     @ 23,39 SAY  "      ≥              ≥        "
     @ 24,38 SAY "‘ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕæ" // utf-8: "‚ïò‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïß‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïß‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïê‚ïõ"
     SETCOLOR( aClr[2] )
     @ 22,39 SAY  " 1005 ≥ Harry Pitts  ≥  78.95 " // utf-8: " 1005 ‚îÇ Harry Pitts  ‚îÇ  78.95 "
     SETCOLOR( aClr[5] )
     @ 23,39 SAY  " 3162 "
     @ 23,46 SAY         " Barb Wire    "
     @ 23,61 SAY                        " 345.06 "

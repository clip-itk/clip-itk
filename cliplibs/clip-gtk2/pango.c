...
CLIP_DLLEXPORT GtkType _gtk_type_pango_context() { return GTK_TYPE_PANGO_CONTEXT; }

long _clip_type_pango_layout() { return GTK_OBJECT_PANGO_LAYOUT; }
long _clip_type_pango_context() { return GTK_OBJECT_PANGO_CONTEXT; }

const char * _clip_type_name_pango_layout()  { return "GTK_OBJECT_PANGO_LAYOUT"; }
const char * _clip_type_name_pango_context()  { return "GTK_OBJECT_PANGO_CONTEXT"; }
...
{
	_wtype_table_put(_clip_type_pango_context,  _clip_type_name_pango_context,  _gtk_type_pango_context, NULL, NULL);
	return 0;
}
...
clip_PANGOLAYOUTNEW(ClipMachine * cm)
{
	...
	if (layout)
	{
		...
		if (!clayout) clayout = _register_object(cm,layout,GTK_TYPE_PANGO_LAYOUT,NULL,NULL);
...
clip_PANGOLAYOUTGETCONTEXT(ClipMachine * cm)
{
	...
	if (context)
	{
		...
		if (!ccontext) ccontext = _register_object(cm,context,GTK_TYPE_PANGO_CONTEXT,NULL,NULL);
...
clip_PANGOLAYOUTGETATTRIBUTES(ClipMachine * cm)
{
	...
	if (pattrs)
	{
		...
		if (!cpattrs) cpattrs = _register_object(cm, pattrs, GTK_TYPE_PANGO_ATTR_LIST, NULL, NULL);
		if (cpattrs) _clip_mclone(cm, RETPTR(cm), &cpattrs->obj );
	}
	return 0;
err:
	return 1;
...
clip_PANGOLAYOUTGETTABS(ClipMachine * cm)
{
	...
	if (tabs)
	{
		...
		if (!ctabs) ctabs = _register_object(cm, tabs, GTK_TYPE_PANGO_TAB_ARRAY, NULL, NULL);
		if (ctabs) _clip_mclone(cm, RETPTR(cm), &ctabs->obj );
	}
	return 0;
err:
	return 1;
...
CLIP_DLLEXPORT int _map_to_pango_rectangle (ClipMachine *cm, ClipVar *map, PangoRectangle *pos)
{
...
clip_PANGOLAYOUTGETLINES(ClipMachine * cm)
{
	...
	if (list)
	{
		...
		for(l=0; l<i; l++, list=g_slist_next(list))
		{
			...
			cline = _list_get_cobject(cm, line);
			if (!cline) cline = _register_object(cm, line, GTK_TYPE_PANGO_LAYOUT_LINE, NULL, NULL);
			if (cline) _clip_aset(cm, cv, &cline->obj, 1, &l);
		}
	}
	return 0;
err:

...
?
? "*************************************************************************"
? "*** Sorry. In this moment test TBROW don't work with gtk+-2.0 library ***"
? "*************************************************************************"
?
?
...
gtk_WindowSetDefaultSize(w, 550, 300)
gtk_WidgetRealize(w)

frame := array(cols)
frameHb := array(cols)
frameFb := array(cols)
items := array(__ITEMS__, cols)
...
basestyle:= gtk_WidgetGetModifierStyle(w)
...
//gtk_WidgetModifyStyle(basestyle)
...
for j:= 1 to cols
	...
	gtk_WidgetSetSizeRequest(header[j], headerWidth[j])
	gtk_WidgetModifyStyle(header[j], headstyle)
	...
	next
	gtk_WidgetSetSizeRequest(footHeader[j], headerWidth[j])
	gtk_WidgetModifyStyle(footHeader[j], footHeadstyle)
...
for i=startR to rows
	for j:= startC to cols
		...
		entry[i][j]:i := i
		entry[i][j]:j := j
		gtk_SignalConnect(entry[i][j], GTK_CLICKED_SIGNAL, {|w| rWin := w:i, cWin := w:j, qout(gtk_buttonGetLabel(w))})
		gtk_buttonsetrelief(entry[i][j], GTK_RELIEF_NONE)
		gtk_boxpackStart(box[j], entry[i][j])
		gtk_WidgetSetSizeRequest(box[j], headerWidth[j])
...
if i+sI-1==2
	gtk_WidgetModifyStyle(entry[i][j], style)
else
	gtk_WidgetModifyStyle(entry[i][j], basestyle)
endif
...
if fr_entry==NIL
	...
	fr_items := array(__ITEMS__, fr_cols)
...
gtk_WidgetSetSizeRequest(fr_header[j], fr_headerWidth[j])
gtk_WidgetModifyStyle(fr_header[j], headstyle)
...
next
gtk_WidgetSetSizeRequest(fr_footHeader[j], fr_headerWidth[j])
gtk_WidgetModifyStyle(fr_footHeader[j], footHeadstyle)
...
for i=startR to rows
	for j:= 1 to fr_cols
		if (i+sI-1)<=len(fr_items) .and. j<=fr_cols
			...
			next
			if i==startR
				gtk_WidgetSetSizeRequest(fr_box[j], fr_headerWidth[j])
				gtk_WidgetSetSizeRequest(fr_header[j], fr_headerWidth[j])
				gtk_WidgetSetSizeRequest(fr_footHeader[j], fr_headerWidth[j])
			endif
...
for i=startR to rows
	for j := startC to fr_cols
		...
		fr_entry[i][j]:i := i
		fr_entry[i][j]:j := j
		gtk_SignalConnect(fr_entry[i][j], GTK_CLICKED_SIGNAL, {|w| rWin := w:i, cWin := w:j, qout(gtk_buttonGetLabel(w))})

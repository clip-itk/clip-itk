...
int
clip_GTK_WIDGETDRAWDEFAULT(ClipMachine * cm)
{
	C_widget *cwid = _fetch_cw_arg(cm);
	if (!cwid || !cwid->widget) goto err;
...
int
clip_GTK_WIDGETLOCKACCELERATORS(ClipMachine * cm)
{
	C_widget *cwid = _fetch_cw_arg(cm);
	CHECKCWID(cwid,GTK_IS_WIDGET);
...
int
clip_GTK_WIDGETUNLOCKACCELERATORS(ClipMachine * cm)
{
	C_widget *cwid = _fetch_cw_arg(cm);
	CHECKCWID(cwid,GTK_IS_WIDGET);
...
int
clip_GTK_WIDGETACCELERATORSLOCKED(ClipMachine * cm)
{
	C_widget *cwid = _fetch_cw_arg(cm);
	CHECKCWID(cwid,GTK_IS_WIDGET);
	_clip_retl(cm, gtk_widget_accelerators_locked(cwid->widget));
	return 0;
err:
	return 1;
}
...
/*  */
int
clip_GTK_WIDGETPOPUP(ClipMachine * cm)
{
	C_widget *cwid = _fetch_cw_arg(cm);
	gint         x = _clip_parni(cm,2);
	gint         y = _clip_parni(cm,3);
...
clip_GTK_WIDGETGETNAME(ClipMachine * cm)
{
	C_widget *cwid = _fetch_cw_arg(cm);
	CHECKCWID(cwid,GTK_IS_WIDGET);
	_clip_retc(cm, gtk_widget_get_name(cwid->widget));
	return 0;
err:
	return 1;
}

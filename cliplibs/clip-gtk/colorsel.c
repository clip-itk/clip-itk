...
int
clip_GTK_COLORSELECTIONSETOPACITY(ClipMachine *cm)
{
	C_widget      *ccsel = _fetch_cw_arg(cm);
...
clip_GTK_COLORSELECTIONDIALOGNEW(ClipMachine * cm)
{
	...
	cvbox = _register_widget(cm, GTK_COLOR_SELECTION_DIALOG(wid)->main_vbox, NULL);
	cokbtn = _register_widget(cm, GTK_COLOR_SELECTION_DIALOG(wid)->ok_button, NULL);
	cresetbtn = _register_widget(cm, GTK_COLOR_SELECTION_DIALOG(wid)->reset_button, NULL);
	...
	if (cvbox) _clip_madd(cm,&cwid->obj,HASH_MAINVBOX,&cvbox->obj);
	if (cokbtn) _clip_madd(cm,&cwid->obj,HASH_OKBUTTON,&cokbtn->obj);
	if (cresetbtn) _clip_madd(cm,&cwid->obj,HASH_RESETBUTTON,&cresetbtn->obj);
